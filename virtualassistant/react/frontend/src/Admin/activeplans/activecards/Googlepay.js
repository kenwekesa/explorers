...
// import React, { useState, useEffect } from 'react';
// import { db } from '../../../firebase/firebase';
// import { getDocs, collection, where, query, doc, updateDoc, arrayUnion } from 'firebase/firestore';
// import { useNavigate } from 'react-router-dom';
// import "./PlansData.css";
...
//       await updateDoc(orderDocRef, {
//         vas: arrayUnion(userId)
//       });

//       // Update the local vas state
//       setVas((prev) => [...prev, userId]);
...
import React, { useState, useEffect } from 'react';
import { db } from '../../../firebase/firebase';
import { getDocs, collection, where, query, doc, updateDoc, arrayUnion, arrayRemove } from 'firebase/firestore';
import { useNavigate } from 'react-router-dom';
import "./PlansData.css";
...
const Bidding = ({ isOpen, onClose, id }) => {
  ...
  const handleAssignPlan = async (userId, index) => {
    try {
      ...
      // Check if user_id already exists in vas array
      if (vas.includes(userId)) {
        setErrorMessage('Va has been assigned already.');
        return;
      }

      const orderDocRef = doc(db, 'serviced', id);

      // Update the vas array using arrayUnion
      await updateDoc(orderDocRef, {
        vas: arrayUnion(userId)
      });
